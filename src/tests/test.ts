import {DylibLoader, DylibBase} from "..";
...
const blacklistedGuilds: string[] = [];

const dylibLoader: DylibLoader<EventType, MainType, TestPlugin> = new DylibLoader({
    rawPluginValidator: ((raw: any) => typeof raw.isCrazy === "function") as any,
...
class Testing {
    public on: any  = (property: string, handler: () => any) => {
...
const tests: {
    name: string,
    notes?: string,
    func: (dylibLoader: DylibLoader<EventType, MainType, TestPlugin>) => Promise<any>,
}[] = [
    {
        name: "420 event",
     ...
     */
    {
        ...
        async func(dylibLoader) {
            const updateInternalListeningState: (plugin: TestPlugin, value: boolean) => void = (dylibLoader as any).updateInternalListeningState.bind(dylibLoader);
